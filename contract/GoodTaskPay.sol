...
/**
 * @title GoodTaskPay
 * @notice G$ escrow for Farcaster bounty tasks on Celo.
 *         Creators deposit G$ tied to a task ID.
 *         After admin verification, eligible users claim their share via backend-signed messages.
 */
contract GoodTaskPay is Ownable, ReentrancyGuard, EIP712 {
    ...
    constructor(
        address _signer,
        address _gdollar
    ) Ownable(msg.sender) EIP712("GoodTaskPay", "1") {
        ...
        supportedTokens[_gdollar] = true;
        depositLimits[_gdollar] = DepositLimits({
            minAmount: 1e18,
            maxAmount: 50000e18
        });

        emit TokenSupported(_gdollar, true);
        emit DepositLimitsUpdated(_gdollar, 1e18, 50000e18);
    }

    // ─── Core Functions ──────────────────────────────────────────────────

    /**
     * @notice Creator deposits G$ for a task. Must approve this contract first.
